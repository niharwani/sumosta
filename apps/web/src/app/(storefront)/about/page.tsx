...
const STORY_CHAPTERS = [
  {
    ...
    imageRight: false,
    image: '/images/yatin.jpeg',
    imageAlt: 'Yatin Narechania — Co-founder',
    image2: '/images/rishabh.jpeg',
    imageAlt2: 'Rishabh Makharia — Co-founder',
    imagePosition: 'center 20%',
    imagePosition2: 'center top',
  },
  {
    number: '02',
    ...
    imageAlt: 'Raw honeycomb in a stoneware bowl with wooden dipper — Sumo-inspired minimalism',
  },
  {
    number: '03',
    ...
    imageAlt: 'Extreme macro of raw wild honey with visible pollen and propolis particulate',
  },
  {
    number: '04',
    ...
    imageAlt: 'Five SUMOSTA honeys arranged with forest elements — single-origin Indian ecosystems',
  },
  {
    number: '05',
    ...
    imageRight: false,
    image: '/images/our-story/5.png',
    imageAlt: 'SUMOSTA Wild Forest honey being drizzled onto sourdough — a daily ritual',
  },
];
...
export default function AboutPage() {
  ...
  return (
    <div className="bg-cream text-charcoal font-satoshi min-h-screen">
      ...
      {/* Dark hero */}
      <section className="relative h-[48vh] min-h-[340px] md:h-[60vh] md:min-h-[420px] bg-midnight overflow-hidden">
        <Image
        ...
        />
        <div className="relative h-full flex items-center justify-center text-center px-5 md:px-6">
          <div data-reveal>
            <p className="font-satoshi uppercase tracking-[0.2em] text-honey-300 font-bold text-[11px] md:text-[13px] mb-3 md:mb-4">
              Our Story
            </p>
            <h1 className="font-clash font-extrabold text-cream leading-[1.1] m-0" style={{ fontSize: 'clamp(1.75rem,6vw,4.5rem)' }}>
      ...
      {/* Story chapters — mobile: consistent image → text rhythm across all
          five chapters (natural DOM order). Desktop: alternates image left /
          right via CSS `order` in the sum-story-row media query below. */}
      <div className="max-w-content mx-auto px-5 md:px-6 py-14 md:py-20">
        {STORY_CHAPTERS.map((ch, i) => (
          <div
            key={i}
            data-image-right={ch.imageRight ? 'true' : 'false'}
            className={`sum-story-row grid grid-cols-1 gap-6 md:gap-10 ${
              i < STORY_CHAPTERS.length - 1 ? 'mb-14 md:mb-24' : ''
            }`}
          >
            <div data-reveal className="sum-story-image">
              {ch.image2 ? (
                <div className="relative aspect-[4/3] rounded-xl md:rounded-2xl overflow-hidden grid grid-cols-2 gap-1.5 md:gap-2 bg-sand">
                  <div className="relative">
                    <Image
                      ...
                      style={{ objectFit: 'cover', objectPosition: ch.imagePosition ?? 'center' }}
                    />
                  </div>
                  <div className="relative">
                    <Image
                      ...
                      alt={ch.imageAlt2 ?? ''}
                      sizes="(min-width: 768px) 25vw, 50vw"
                      style={{ objectFit: 'cover', objectPosition: ch.imagePosition2 ?? 'center' }}
                    />
                  </div>
                </div>
              ) : (
            ...
            <div
              data-reveal
              className="sum-story-text flex flex-col justify-center"
            >
              <div className="flex items-center gap-2.5 md:gap-3 mb-3 md:mb-4">
                <span className="font-clash font-extrabold text-sand text-4xl md:text-5xl leading-none">
                  {ch.number}
                </span>
              ...
              <h2
                className="font-clash font-extrabold text-charcoal mb-4 md:mb-6 whitespace-pre-line leading-[1.15]"
              ...
              {ch.paragraphs.map((para, pi) => (
                <p
                  key={pi}
                  className={`font-satoshi text-[14px] md:text-[15px] leading-[1.65] md:leading-[1.8] text-bark ${
                    pi < ch.paragraphs.length - 1 ? 'mb-3 md:mb-4' : ''
                  }`}
                >
                  {para}
                </p>
      ...
      {/* Pull quote — compact */}
      <div className="bg-cream-warm px-5 md:px-6 py-10 md:py-12">
        <div data-reveal className="max-w-[760px] mx-auto text-center">
          <p className="font-bespoke text-honey-500 text-4xl md:text-5xl leading-none mb-2 font-bold" aria-hidden="true">
            &ldquo;
          </p>
          <p
            className="font-bespoke italic text-charcoal leading-[1.55] md:leading-[1.7] m-0"
          ...
          <p className="font-satoshi text-[12px] md:text-[13px] text-earth mt-3 md:mt-4 font-semibold">
            — SUMOSTA Founders
          </p>
        </div>
      </div>
      ...
      {/* Founders */}
      <div className="bg-cream py-14 md:py-20 px-5 md:px-6">
        <div className="max-w-content mx-auto">
          <div data-reveal className="text-center mb-10 md:mb-16">
            <p className="font-satoshi uppercase tracking-[0.15em] text-honey-500 font-bold text-[11px] md:text-xs mb-3 md:mb-4">
              Meet the Founders
            </p>
            <h2
              className="font-clash font-extrabold text-charcoal mb-4 md:mb-5 leading-[1.15]"
              style={{ fontSize: 'clamp(1.5rem,4vw,3.2rem)' }}
            >
              Meet the Heavyweights Behind the Honey.
            </h2>
            <p className="font-satoshi text-sm md:text-base text-bark max-w-[680px] mx-auto leading-[1.6] md:leading-[1.75]">
          ...
          <div className="sum-founders-grid grid grid-cols-1 gap-6 md:gap-8 items-stretch auto-rows-fr">
            {FOUNDERS.map((f) => (
              <div
                key={f.name}
                data-reveal
                className="bg-cream-warm rounded-[16px] md:rounded-[20px] border border-sand p-6 md:p-10 flex flex-col gap-4 md:gap-5 h-full"
              >
                <div className="flex items-center gap-4 md:gap-5">
                  <div
                    className="relative w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden shrink-0 border-2 border-honey-400 bg-honey-200"
                  >
                    <Image
                      src={f.image}
                      fill
                  ...
                  <div>
                    <h3 className="font-clash font-extrabold text-charcoal text-lg md:text-xl mb-0.5">
                      {f.name}
                    </h3>
                    <p className="font-satoshi text-[12px] md:text-[13px] text-honey-500 font-bold mb-1">
                      {f.title}
                    </p>
                    <span className="inline-block bg-honey-100 text-honey-600 font-satoshi text-[10px] md:text-[11px] font-bold px-2 md:px-2.5 py-0.5 rounded-full tracking-[0.05em]">
                      {f.badge}
                    </span>
                  </div>
                </div>
                <div className="bg-cream rounded-lg px-3 md:px-4 py-2.5 md:py-3 border-l-[3px] border-honey-400">
                  <p className="font-satoshi uppercase tracking-[0.12em] text-honey-500 font-bold text-[10px] md:text-[11px] mb-1">
                    The Sumo Stance
                  </p>
                  <p className="font-satoshi text-[13px] md:text-sm text-bark leading-[1.55] md:leading-[1.6] m-0">{f.stance}</p>
                </div>
                <p className="font-satoshi text-[14px] md:text-[15px] text-bark leading-[1.65] md:leading-[1.8] m-0">{f.profile}</p>
                <p className="font-satoshi text-[12px] md:text-[13px] text-earth m-0 mt-auto">
          ...
          <div
            data-reveal
            className="text-center mt-10 md:mt-14 px-5 md:px-6 py-8 md:py-10 bg-charcoal rounded-[16px] md:rounded-[20px]"
          >
            <p
              className="font-bespoke italic text-cream leading-[1.6] md:leading-[1.75] max-w-[700px] mx-auto m-0"
              style={{ fontSize: 'clamp(0.9375rem,1.8vw,1.25rem)' }}
            >
            ...
            <p className="font-satoshi text-[12px] md:text-[13px] text-honey-400 mt-4 md:mt-5 font-semibold">
      ...
      {/* CTA */}
      <div className="bg-charcoal py-14 md:py-20 px-5 md:px-6 text-center">
        <h2
          className="font-clash font-extrabold text-cream mb-4 md:mb-5 leading-[1.15]"
          style={{ fontSize: 'clamp(1.5rem,3.5vw,2.8rem)' }}
        >
          Ready to taste the difference?
        </h2>
        <p className="font-satoshi text-sm md:text-base text-earth-light mb-6 md:mb-8">
      ...
      <style>{`
        ...
        @media (min-width: 768px) {
          ...
          /* Desktop-only alternation. On mobile, DOM order (image, then text)
             is preserved so the whole page reads image → text × 5. */
          .sum-story-row[data-image-right="true"]  .sum-story-image { order: 2; }
          .sum-story-row[data-image-right="true"]  .sum-story-text  { order: 1; }
          .sum-story-row[data-image-right="false"] .sum-story-image { order: 1; }
          .sum-story-row[data-image-right="false"] .sum-story-text  { order: 2; }
        }
      `}</style>
    </div>
  );
